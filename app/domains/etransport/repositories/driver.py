...
from domains.etransport.schemas.driver import (
...
class CRUDDriver(CRUDBase[Driver, DriverCreate, DriverUpdate]):
    ...
    def is_email_taken(self, db: Session, email: str, exclude_id: UUID4) -> bool:
        ...
        query = db.query(Driver).filter(Driver.email == email)
        if exclude_id: query = query.filter(Driver.id != exclude_id)
    ...
    ## function to update admin or Drivers password base on token after resetting password
    def update_Driver_after_reset_password(
            self, db: Session, *,
            db_obj: ModelType,
            data: Union[DriverUpdate, Dict[str, Any]]
    ):
...
driver_actions = CRUDDriver(Driver)
